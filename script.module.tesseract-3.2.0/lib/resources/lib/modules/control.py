...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.tesseract.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.tesseract.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.tesseract.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.tesseract.artwork)')
...
def metaFile():
    if condVisibility('System.HasAddon(script.tesseract.metadata)'):
        return os.path.join(xbmcaddon.Addon('script.tesseract.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
